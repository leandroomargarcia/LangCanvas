// Free-tier Gemini cascade. Flash first (better drafts), then Flash-Lite ids.
// Each model has its own RPM/RPD on a Free project
// (see https://ai.google.dev/gemini-api/docs/rate-limits). After 429/404/503
// the next id uses a separate daily bucket instead of failing or jumping to Pro.

export const LITE_TIER_MODELS = [
...
export async function generateGeminiText({
  ...
  const queue = Array.isArray(models) && models.length ? models : FREE_TIER_MODELS;
  let lastDetail = '';
  for (const model of queue) {
    ...
    if (!data) {
      lastDetail = model + ' empty JSON body';
      continue;
    }
