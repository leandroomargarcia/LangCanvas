...
import { evalPredicate, formatPredicate, missingWaitKeys } from './predicate.js';
...
export async function runGraph(graph, input, adapters) {
  ...
  async function resolveJoin(joinId) {
    ...
    for (let i = 0; i < preds.length; i++) {
      const pid = preds[i];
      if (pid === start.id || waveRan.has(pid)) continue;
      const pred = nodeById(graph, pid);
      if (!pred || pred.type === 'start') continue;
      const r = await walkFrom(pid, true);
      if (r && r.type === 'error') return r;
    }
    const unread = preds.filter(pid => {
      const pred = nodeById(graph, pid);
      if (!pred || pred.type === 'start') return false;
      return !waveRan.has(pid);
    });
    if (unread.length) {
      const names = unread.map(pid => (nodeById(graph, pid) || {}).label || pid);
      const note = 'waiting for ' + names.join(', ');
      trace.push({ label: n.label, type: 'join', note });
      return { type: 'error', error: 'Join "' + n.label + '" is ' + note + '.' };
    }
    const keys = waitKeysOf(n);
    const missing = missingWaitKeys(state, keys);
    if (missing.length) {
      const note = 'holding first report; still waiting for ' + missing.join(', ');
      trace.push({ label: n.label, type: 'join', note, reads: snapshot(state, keys) });
      return { type: 'error', error: 'Join "' + n.label + '" is waiting for: ' + missing.join(', ') + '.' };
    }
    const ready = keys.length ? 'all reports ready: ' + keys.join(', ') : 'all incoming branches arrived';
    trace.push({
      label: n.label,
      type: 'join',
      note: ready,
      reads: snapshot(state, keys.length ? keys : (n.detail && n.detail.reads) || []),
    });
