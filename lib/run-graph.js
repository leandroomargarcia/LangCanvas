...
export function mockStructured(schemaFields, input) {
...
function evalPredicate(state, d) {
  if (!d) return false;
  const leftRaw = state[d.predLeft];
  const op = d.predOp || '>=';
  if (op === 'truthy') return !!leftRaw && leftRaw !== 'false' && !(Array.isArray(leftRaw) && !leftRaw.length);
  if (op === 'empty') return leftRaw == null || leftRaw === '' || (Array.isArray(leftRaw) && !leftRaw.length);
  const rightRaw = d.predRightMode === 'literal' ? d.predRight : state[d.predRight];
...
function applyWrites(state, writes, payload) {
  ...
  (writes || []).forEach(w => {
    ...
    if (payload.args && Object.prototype.hasOwnProperty.call(payload.args, w.key)) {
      state[w.key] = payload.args[w.key];
      wrote[w.key] = state[w.key];
      return;
    }
    if (w.op === 'set' && payload.args && w.key === 'search_queries' && payload.args.search_queries) {
      state.search_queries = payload.args.search_queries;
      wrote.search_queries = state.search_queries;
    }
  });
  if (payload.args) {
    ['search_queries', 'answer', 'documents', 'notes', 'draft'].forEach(k => {
      if (payload.args[k] != null && state[k] !== undefined && wrote[k] == null) {
        state[k] = payload.args[k];
        wrote[k] = state[k];
      }
    });
  }
...
export async function runGraph(graph, input, adapters) {
  ...
  const llm = adaptersSafe.llm || (async ({ fields, input: q }) => mockStructured(fields, q));
  ...
  while (id && visits < MAX_VISITS) {
    ...
    if (n.type === 'router') {
      ...
      if (d.stopMode === 'tool_rounds') {
        ...
        reason = 'predicate ' + (d.condition || d.predLeft || '') + ' → ' + shouldStop;
      }
...
export const defaultMockAdapters = {
  llm: async ({ fields, input }) => mockStructured(fields, input),
