...
import { evalPredicate, formatPredicate, isEmptyValue, missingWaitKeys, constValueByKey, constsBag } from './predicate.js';
...
export function evalWriteExpr(expr, ctx) {
  ...
  if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(e)) {
    const cv = constValueByKey((ctx && ctx.consts) || [], e);
    if (cv !== undefined) return { ok: true, value: cv };
  }
...
export function interpolatePrompt(template, state, extra) {
  ...
  if (!src.trim()) return '';
  const extraBag = extra && typeof extra === 'object' ? extra : {};
  const fromConsts = constsBag(extraBag.consts);
  const rest = {};
  Object.keys(extraBag).forEach(k => {
    if (k !== 'consts') rest[k] = extraBag[k];
  });
  const bag = Object.assign({}, fromConsts, state || {}, rest);
...
async function runAction(graph, n, state, adapters, counters) {
  ...
  if (kind === 'llm') {
    ...
    const prompt = interpolatePrompt(promptTemplate, state, extra);
    const tempRaw = d.temperature;
    const temperature = tempRaw == null || tempRaw === '' ? null : Number(tempRaw);
    const args = await adapters.llm({
      system: interpolatePrompt(d.system || 'Fill the output schema. Return JSON only.', state, extra),
      promptTemplate,
      prompt,
      schemaName,
      fields,
    ...
    wrote = applyWrites(state, d.writes, { message, args, consts: graph.consts });
