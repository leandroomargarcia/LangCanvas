...
export function formatCoachUserMessage(pack, graphText, question) {
  ...
  const roster = pack.pattern && PATTERNS[pack.pattern]
    ...
    : '(custom — roles are the labels on CURRENT CANVAS plus “problem to solve”. Do not invent a template roster. Do not tell them to delete specialists.)';
  return [
    'GOAL (problem to solve): ' + (pack.goal || '(empty — ask them to fill the panel)'),
    'MODE: ' + pack.mode,
    'PATTERN: ' + (pack.patternName || 'custom') + (pack.pattern ? ' [' + pack.pattern + ']' : ''),
    'CURRENT PHASE: ' + (pack.phase || 1) + ' ' + (pack.phaseName || 'diagram') + ' (1 diagram → 2 dictionary → 3 configure → 4 review → 5 run)',
    pack.wire ? ('CANONICAL WIRE: ' + pack.wire) : 'CANONICAL WIRE: (none — wire THEIR boxes; do not replace them with Self-RAG / Reflexion)',
    '',
    'BUILD SEQUENCE (stay on CURRENT PHASE. Chat history is the thread — continue it. Do not restart unless they said Clean.):',
    '1 diagram: complete nodes, arrows, one-line effects. Custom graphs keep supervisor/workers. Template only if MODE=pattern.',
    '2 dictionary: SHARED STATE + SCHEMAS. For every key/schema: what it stores, who writes it, who reads it.',
    '3 configure: NODE DETAIL one node at a time (kind, output schema, reads/writes, stop when). Explain each field.',
    '4 review: debug mismatches vs GOAL. Do not wipe the design.',
    '5 run: panel execution → input → Dry run → Run. Read history.',
    '',
    'ROSTER (only if they ask what to create, and only in phase 1):',
    roster,
    '',
    'LANGCANVAS UI FOR THIS STEP:',
    pack.uiManual,
    '',
    'FIELD GLOSSARY:',
    FIELD_GLOSSARY,
    '',
    'CURRENT GAP (teach this step only):',
    'id: ' + (g.id || ''),
    'why: ' + (g.why || ''),
    'clicks:\n' + clicks,
