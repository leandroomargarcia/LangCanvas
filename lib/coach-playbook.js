...
Reads/writes: only the keys this role truly needs.`,
  ...
  router: `Select the conditional → NODE DETAIL.
Stop must be countable, not "is the answer good?".
Modes: state predicate (e.g. attempts >= max) OR N tool rounds (count tool results in messages; N = a state int key).
Two outgoing arrows, labeled (yes/no, continue/end). Swapped arrows kill the loop.
End does not compute — it only cuts.`,
...
Click an edge to label a branch.
...
function routers(g) {
  return (g.nodes || []).filter(n => n.type === 'router');
}
...
export const PATTERNS = {
  ...
  guardrails: {
    ...
    wire: 'start → input safe? yes → agent → output ok? yes → end. no → refuse / rewrite',
    roles: [
...
const MATCH_ORDER = [
  ...
  'supervisor', 'guardrails', 'hitl', 'retry', 'rag', 'react', 'simple',
];
...
function patternRouterGap(g, spec, id) {
  ...
  if (r.mode === 'tool_rounds' && n.stopMode !== 'tool_rounds') {
    return gap('router', 'router', 'Do not stop on “is the answer good?”. Count tool rounds in messages.', [
      'Click **' + name + '**. Stop when: **N tool rounds**. N = state key **' + (r.maxKey || 'max_iterations') + '**.',
      'Continue → the investigate/tools node. Stop → end. If those arrows are swapped, the agent dies on the first critique.',
    ], { pattern: id, mode: 'pattern' });
  }
  if (r.mode === 'predicate' && n.stopMode === 'tool_rounds') {
    return gap('router', 'router', 'This pattern stops on a state predicate, not tool rounds.', [
      'Click **' + name + '**. Stop when: state predicate. Left: **' + (r.left || 'iterations') + '**.',
    ], { pattern: id, mode: 'pattern' });
  }
  return null;
}
...
function customGap(g) {
  ...
  const weakR = routers(g).find(n => outsFrom(g, lab(n)).length < 2 || (n.stopMode === 'predicate' && !(n.condition || '').trim() && n.stopMode !== 'tool_rounds'));
  if (weakR) {
    return gap('router', 'router', 'Give the conditional a real stop and two labeled exits.', [
      'Click **' + lab(weakR) + '**. Predicate on a state key, or N tool rounds, or a human yes/no — not “is it good?”.',
      'Two outgoing arrows, labeled so continue vs stop cannot be swapped.',
    ], { mode: 'custom' });
  }
