...
On a conditional, the "if" row can compare a SHARED STATE key against a constant (pick "constant", then the name).`,
...
- If "N tool rounds": row "N" → "state key", "constant", or "literal". Counts tool results in messages; no extra counter.
- If "state predicate": row "if" is dropdowns. First pick the SHARED STATE key, then the operator. Operators include "is empty", "is not empty", "is truthy", and comparisons. The right side can be another state key, a constant (MAX_ATTEMPTS), or a literal. "+ AND another check" adds a second test on the same router. Then pick "match all (AND)" or "match any (OR)". Never a Python text box. Never len(...).
...
Conditional panel: "stop when", "N", "if", "then return", "state predicate", "N tool rounds", "state key", "constant", "literal", "other key", "is empty", "is not empty", "+ AND another check", "match all (AND)", "match any (OR)".
