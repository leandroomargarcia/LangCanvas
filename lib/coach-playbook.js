...
bind_tools + tool_choice is ONLY structured output to that schema. It does NOT attach Tavily, calculate_cost, or any catalog tool. Catalog tools are separate kind=tool action nodes on the canvas.`,

  state: `PANEL SHARED STATE.
Memory the next node still needs. Not a copy of every schema field.
Each row: key, type (str / int / float / bool / list / list[str] / messages), val.
Agent graphs almost always need messages (the thread).
Project onto state only what a tool must read (list key = batch) and counters that change during the run (attempts).
Retry ceilings like MAX_ATTEMPTS do NOT go here — they go in constants.
Delete leftover keys that the design does not use.`,

  consts: `PANEL CONSTANTS (left, under shared state).
Not graph memory. These become consts.py, not TypedDict fields.
Each row: name (e.g. MAX_ATTEMPTS), type (str / int / float / bool), val.
Use this for budgets, limits, and other numbers that never change during a run.
On a conditional, the "if" row value dropdown lists SHARED STATE and CONSTANTS together — pick MAX_ATTEMPTS under constants. Same for N tool rounds.`,
...
System prompt: one role line. Keep bind_tools on (schema lock, not catalog tools).
Never tell the student that bind_tools lets the LLM “call Tavily internally”. If Tavily / cost must run, that is a tool node plus arrows (often a conditional like reporte_ok? that loops back to the SAME LLM).`,
...
- If "N tool rounds": row "N" → "value" (shared state or a constant like MAX_ITERATIONS) or "literal". Counts tool results in messages; no extra counter.
- If "state predicate": row "if" is dropdowns. First pick the SHARED STATE key, then the operator. Operators include "is empty", "is not empty", "is truthy", and comparisons. The right side is "value" (shared state keys and constants in one list) or "literal". Pick MAX_ATTEMPTS under constants. "+ AND another check" adds a second test on the same router. Then pick "match all (AND)" or "match any (OR)". Never a Python text box. Never len(...).
...
Conditional panel: "stop when", "N", "if", "then return", "state predicate", "N tool rounds", "value", "literal", "is empty", "is not empty", "+ AND another check", "match all (AND)", "match any (OR)". The value dropdown lists shared state and constants together.
Join panel: "wait until present", "use incoming writes". + join is the barrier.
LLM panel: kind LLM, "output schema", reads, writes. bind_tools = force that schema; NOT Tavily.
Tool panel: kind tool, catalog tool, map arg → state key, "runs when LLM returns".
Constants panel: budgets like MAX_ATTEMPTS — not shared state.
...
function constKey(g, re) {
  return (g.consts || []).find(v => re.test(v.key || ''));
}
...
function patternConstsGap(g, spec, id) {
  const need = spec.consts || [];
  const leftover = (g.state || []).find(v => /^(max_attempts|max_iterations)$/i.test(v.key || ''));
  if (leftover) {
    const dest = /attempts/i.test(leftover.key) ? 'MAX_ATTEMPTS' : 'MAX_ITERATIONS';
    return gap('consts', 'consts', '**' + leftover.key + '** does not change during the run. That is a constant, not shared state.', [
      'CONSTANTS → + constant **' + dest + '** type int' + (need[0] && need[0].val ? ' val ' + need[0].val : '') + '.',
      'Delete **' + leftover.key + '** from SHARED STATE.',
      'On the conditional **if** (or row **N**): pick **' + dest + '** under **constants**, not another state key.',
    ], { pattern: id, mode: 'pattern' });
  }
  if (!need.length) return null;
  const missing = need.filter(s => !constKey(g, new RegExp('^' + s.key + '$', 'i')));
  if (!missing.length) return null;
  return gap('consts', 'consts', 'Budgets are constants, not shared state. They never change during a run.', [
    'CONSTANTS: ' + need.map(s => '**' + s.key + '** type ' + s.type + (s.val ? ' val ' + s.val : '')).join('; ') + '.',
    'On budget spent? / should_continue / event_loop: pick that name under **constants** in the value dropdown.',
  ], { pattern: id, mode: 'pattern' });
}
...
function patternRouterGap(g, spec, id) {
  ...
  if (r.mode === 'tool_rounds' && stopKind(n) !== 'tool_rounds') {
    return gap('router', 'router', 'Do not stop on “is the answer good?”. Count tool rounds in messages.', [
      ...
      'Row **N**: **value**, pick **' + (r.maxKey || 'MAX_ITERATIONS') + '** (under constants if it is a budget).',
  ...
  if (r.mode === 'tool_rounds' && stopKind(n) === 'tool_rounds' && !stopNKey(n)) {
    return gap('router', 'router', 'N tool rounds still needs the budget key.', [
      'On **' + name + '**, row **N**: **value** → **' + (r.maxKey || 'MAX_ITERATIONS') + '**.',
    ], { pattern: id, mode: 'pattern' });
  }
  return null;
}
...
function suggestedStateRows(g) {
  ...
  if (findNode(g, /budget|grounded/i)) {
    add('attempts', 'int', 'Incremented each time the generator/classifier retries. budget spent? compares it to MAX_ATTEMPTS.');
  }
...
function suggestedConstRows(g) {
  const rows = [];
  const add = (key, type, why) => {
    if (!rows.some(r => r.key === key)) rows.push({ key, type, why });
  };
  if (findNode(g, /budget/i)) {
    add('MAX_ATTEMPTS', 'int', 'Retry ceiling. budget spent? compares attempts against this constant, not a state key.');
  }
  if (findNode(g, /should_continue|event_loop/i)) {
    add('MAX_ITERATIONS', 'int', 'Loop ceiling. The conditional compares against this constant.');
  }
  return rows;
}
...
function dictionaryGap(g, spec, id) {
  ...
  if (spec) {
    ...
    const cs = patternConstsGap(g, spec, id);
    if (cs) return stamp(cs, 2, 'dictionary');
  ...
  const leftoverCeiling = (g.state || []).find(v => /^(max_attempts|max_iterations)$/i.test(v.key || ''));
  if (leftoverCeiling) {
    const dest = /attempts/i.test(leftoverCeiling.key) ? 'MAX_ATTEMPTS' : 'MAX_ITERATIONS';
    return stamp(gap('consts', 'consts',
      '**' + leftoverCeiling.key + '** does not change during the run. That is a constant, not shared state.',
      [
        'CONSTANTS → + constant **' + dest + '** (type int). Why: the conditional compares against this ceiling.',
        'Delete **' + leftoverCeiling.key + '** from SHARED STATE. On the conditional, pick **' + dest + '** under **constants**.',
      ], extra), 2, 'dictionary');
  }
  const missingConsts = suggestedConstRows(g).filter(s => !constKey(g, new RegExp('^' + s.key + '$', 'i')));
  if (missingConsts.length) {
    return stamp(gap('consts', 'consts',
      'Budgets belong in CONSTANTS so nodes like budget spent? can compare against them.',
      missingConsts.map(s =>
        'CONSTANTS → + constant **' + s.key + '** (type ' + s.type + '). Why: ' + s.why
      ), extra), 2, 'dictionary');
  }
...
function configureGap(g, spec, id) {
  ...
  if (weakR) {
    ...
    const predHint = /grounded/i.test(name)
      ...
      : /budget/i.test(name)
        ? '**if** attempts >= MAX_ATTEMPTS (pick MAX_ATTEMPTS under constants, not shared state; yes → supervisor or end, no → retry the generator).'
...
const REWIRE_QUESTION_RE = /rediseñ|rewire|cambia(r|á)? (el )?(flujo|arquitectura|grafo)|borr(a|á|ar) (el )?join|separ(a|á|ar) (el )?orquest|dos orquest|nuevo (grafo|diseño|join)|reemplaz.*(template|grafo)|hac[eé]lo lineal|pipeline lineal|tools? adentro|elimin[aá] (el )?(join|reporte_ok)|split (the )?(orchestrat|orquest)|(?:quiero|us[aá]|hac[eé]|pascame a).*(self-rag|reflexion|react)/i;

const USER_LOCK_RE = /no (quiero )?(cambiar tanto|separar|separes)|el join (es |sí es |queda |se queda )?necesario|no (lo )?reemplac|do not replace|un solo orquestador|no (quiero )?dos orquest|tools? como (nodos|cajas)|el join se queda|no cambies tanto|manten[eé] (mi |la )?arquitectura|keep (my |the )?(join|architecture|orchestrat)/i;

const PLAN_REWIRE_RE = /borr[aá] (el )?(join|nodo|reporte_ok)|elimin[aá] (el )?(join|reporte_ok|orquestador)|rediseñ|pipeline lineal|flujo lineal|separ[aá].*orquest|orquestador_(final|inicial)|nuevo join|join_pre|bindTools.*Tavily|tools? (adentro|internas|dentro del)|antes del orquestador|delete (the )?(join|conditional)|split (the )?(llm|orchestrat)|absorb.*tool|linear(ize| pipeline)|poner .*tools? dentro|tools before/i;

export function wantsRewire(question) {
  return REWIRE_QUESTION_RE.test(String(question || ''));
}

export function extractDesignLocks(history) {
  const hits = [];
  (history || []).forEach((m) => {
    if (!m || m.role !== 'user') return;
    const t = String(m.text || '').replace(/\s+/g, ' ').trim();
    if (USER_LOCK_RE.test(t)) hits.push(t.slice(0, 280));
  });
  return [...new Set(hits)].slice(-6);
}

function canvasHasSkeleton(graph) {
  const g = graph || {};
  return realActions(g).length >= 2 || (g.edges || []).length >= 2;
}

function nodesInCycle(graph) {
  ...
  const llms = (graph.nodes || []).filter((n) => n.kind === 'llm').map((n) => lab(n)).filter(Boolean);
  const looping = [];
  llms.forEach((start) => {
    const seen = new Set([start]);
    const q = (adj[start] || []).slice();
    let hops = 0;
    while (q.length && hops < 24) {
      const cur = q.shift();
      hops += 1;
      if (cur === start) {
        looping.push(start);
      ...
      seen.add(cur);
      (adj[cur] || []).forEach((n) => q.push(n));
    }
  });
  return [...new Set(looping)];
}

export function architectureLock(graph, question, history) {
  const g = graph || {};
  const skeleton = canvasHasSkeleton(g);
  const rewireRequested = wantsRewire(question);
  const topologyLocked = skeleton && !rewireRequested;

  const nodes = g.nodes || [];
  const edges = g.edges || [];
  const joins = nodes.filter((n) => n.type === 'join' || n.kind === 'join');
  const tools = nodes.filter((n) => n.kind === 'tool');
  const llms = nodes.filter((n) => n.kind === 'llm');
  const routers = nodes.filter((n) => n.type === 'router' || n.kind === 'conditional');
  const looping = nodesInCycle(g);
  const chatLocks = extractDesignLocks([...(history || []), { role: 'user', text: question || '' }]);

  const lines = [
    'DESIGN LOCK — CURRENT CANVAS topology is the contract unless QUESTION explicitly asks to rewire.',
    topologyLocked
      ? 'LOCK ACTIVE: do not add/remove/rename nodes, do not delete join, do not split one LLM into two, do not absorb tool nodes into bind_tools, do not linearize a loop into a pipeline, do not delete reporte_ok? / grounded? / budget spent?. Only fix state, schemas, reads/writes/exprs, prompts, predicates, and wait-until keys on EXISTING nodes. Connecting a dangling existing node to its obvious neighbor is OK; inventing a new architecture is not.'
      : (skeleton
        ? 'QUESTION looks like a rewire request: topology changes are allowed. Still keep their labels and GOAL. Do not swap in a stock template.'
        : 'Canvas is still empty or a stub — you may propose a first architecture from GOAL.'),
    'LANGCANVAS FACT: bind_tools + tool_choice = structured output to that LLM’s output schema (e.g. Reporte). It does NOT attach Tavily or calculate_cost. Catalog tools run only as kind=tool action nodes (arrows + often a conditional that loops back to the SAME LLM).',
    'Nodes: ' + (nodes.map((n) => lab(n) + '/' + (n.kind || n.type)).filter(Boolean).join(', ') || '(none)'),
    'Edges: ' + (edges.map((e) => (e.from || '') + '→' + (e.to || '') + (e.label ? '(' + e.label + ')' : '')).join('; ') || '(none)'),
    joins.length
      ? ('Joins: ' + joins.map((j) => lab(j) + ' waitUntil=[' + ((j.waitUntil || []).join(',') || 'none') + ']').join('; ') + '. A join waits for specialist reports (often ncm_info + hab_info). Do not also wait for later tool outputs (tributos, precio_ref) unless QUESTION asks.')
      : 'No join on canvas.',
    tools.length
      ? ('Tool NODES (graph steps, not bind_tools): ' + tools.map((t) => lab(t) + '=' + (t.tool || '?')).join(', '))
      : 'No tool nodes.',
    llms.length
      ? ('LLM nodes (do not split unless asked): ' + llms.map((n) => lab(n) + (n.outputSchema ? ' schema=' + n.outputSchema : '')).join(', '))
      : 'No LLM nodes.',
    routers.length
      ? ('Conditionals (keep): ' + routers.map((n) => lab(n)).join(', '))
      : 'No conditionals.',
    looping.length
      ? ('Detected LLM loops (keep the same LLM, do not split): ' + looping.join(', '))
      : '',
    chatLocks.length
      ? ('STUDENT AGREEMENTS (harder than PLAN or checker): ' + chatLocks.join(' | '))
      : '',
  ].filter(Boolean);
  return {
    text: lines.join('\n'),
...
export function sanitizeCoachPlan(plan, topologyLocked) {
  if (!plan || !topologyLocked) return plan;
  const bad = (s) => PLAN_REWIRE_RE.test(String(s || ''));
  const issues = Array.isArray(plan.issues) ? plan.issues.filter((s) => !bad(s)) : [];
  const clicks = Array.isArray(plan.next_clicks) ? plan.next_clicks.filter((s) => !bad(s)) : [];
  let focus = String(plan.focus || '');
  if (bad(focus) || /architect|topology|redesign|rewire|pipeline|flujo|linear/i.test(focus)) {
    focus = 'state_writes_debug';
  }
  const stripped = (plan.issues || []).length !== issues.length || (plan.next_clicks || []).length !== clicks.length;
  return {
    goal_in_one_line: plan.goal_in_one_line,
    focus,
    issues,
    next_clicks: clicks,
    lock_note: stripped ? 'ARCHITECTURE LOCK stripped topology-rewriting clicks. Fill state/schemas/writes/debug only.' : '',
  };
}

export function buildCoachPack(graph, question, stickyPattern, history) {
  ...
  const lock = architectureLock(graph, question, history);
  return {
    mode: current.mode || (pattern ? 'pattern' : 'custom'),
    pattern: pattern || '',
    patternName: spec ? spec.name : 'custom architecture',
    wire: lock.topologyLocked ? '' : (spec ? spec.wire : ''),
...
function lockSafeClicks(clicks, topologyLocked) {
  if (!topologyLocked) return clicks || [];
  return (clicks || []).filter((c) => !PLAN_REWIRE_RE.test(String(c || '')));
}
...
export function formatCoachUserMessage(pack, graphText, question, executionText) {
  ...
  const clickList = lockSafeClicks(g.clicks || [], topologyLocked);
  const clicks = clickList.map((c, i) => (i + 1) + '. ' + c).join('\n');
  const roster = topologyLocked
    ? '(topology locked — do not invent template roles or extra LLMs. Configure the nodes already on CURRENT CANVAS.)'
    : pack.pattern && PATTERNS[pack.pattern]
  ...
  const help = topologyLocked
    ? [
        'Help them in this order — SKIP architecture unless QUESTION asks to rewire:',
        '- Shared state: each key — what it stores, who writes it, who reads it.',
        '- Constants: budgets that never change (MAX_ATTEMPTS). Not TypedDict fields.',
        '- Schemas: contract of each LLM shot (not the same as shared state).',
        '- Writes/exprs: the field a later tool or conditional actually reads (e.g. response.search_queries, not an empty state copy).',
  ...
  return [
    pack.lock && pack.lock.text ? pack.lock.text : '',
    '',
    ...
    '',
    'MODE: ' + pack.mode + ' | PATTERN: ' + (pack.patternName || 'custom') + (pack.pattern ? ' [' + pack.pattern + ']' : '') + (topologyLocked ? ' | TOPOLOGY LOCKED' : ''),
    pack.wire ? ('TEMPLATE WIRE (only if they asked for this template): ' + pack.wire) : 'TEMPLATE WIRE: none — do not substitute a stock template.',
    '',
    help,
    '',
    ...
    FIELD_GLOSSARY,
    '',
    'CHECKER HINTS (optional lint — ignore any hint that deletes nodes, splits an LLM, or linearizes a locked loop):',
    'why: ' + ((topologyLocked && PLAN_REWIRE_RE.test(g.why || '')) ? '(suppressed topology rewrite — fill existing fields instead)' : (g.why || '')),
    'clicks:\n' + (clicks || '(none — look at CURRENT CANVAS writes/schemas/execution)'),
    '',
